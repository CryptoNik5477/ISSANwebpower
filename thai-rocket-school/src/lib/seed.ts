// Shared seeding logic — the full Thai Rocket School curriculum: 2 courses,
// 19 levels, ~120 lessons, 19 exams, achievements and demo accounts.
// Used by both the CLI script (prisma/seed.ts) and the protected
// /api/admin/seed route (for environments without local DB access).

import { PrismaClient, LessonType } from "@prisma/client";
import bcrypt from "bcryptjs";
import { l, quizFromVocab, reverseQuizFromVocab, type L, type Vocab } from "../../prisma/seed-data/helpers";
...
/** Idempotent: safe to call multiple times (every write is an upsert / count-guarded create). */
export async function runSeed(prisma: PrismaClient): Promise<SeedSummary> {
  ...
  await prisma.user.upsert({
    where: { email: "admin@thairocketschool.com" },
    update: { role: "ADMIN" },
    create: {
      email: "admin@thairocketschool.com",
      name: "Admin",
      role: "ADMIN",
      plan: "LIFETIME",
  ...
  await prisma.user.upsert({
    where: { email: "demo@thairocketschool.com" },
    update: {},
    create: {
      email: "demo@thairocketschool.com",
      name: "Demo Student",
      role: "STUDENT",
      plan: "YEARLY",
  ...
  return { courses: 2, levels: levelCount, achievements: ACHIEVEMENTS.length };
}
