...
| Role | Email | Password |
|---|---|---|
| Admin | `admin@thairocketschool.com` | `admin1234!` |
| Student | `demo@thairocketschool.com` | `demo1234!` |
...
**Vercel** — import the repo, set the root directory to `thai-rocket-school/`, add the env vars, and attach a Postgres database (Neon/Supabase). The build script pushes the schema automatically; run `npm run db:seed` once via `vercel exec` or a local connection.

**Docker / VPS** — `docker compose up -d --build` behind any reverse proxy (Caddy/Traefik/Nginx) with TLS.

## License

Proprietary — all course content and code © Thai Rocket School.
