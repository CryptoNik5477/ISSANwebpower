...
export async function sendEmail({ to, subject, html }: SendArgs): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? "ThaiMastery <onboarding@resend.dev>";
...
function layout(body: string): string {
  ...
  <table role="presentation" width="100%" style="max-width:560px;margin:0 auto;background:#141a2e;border-radius:16px;overflow:hidden">
    <tr><td style="padding:28px 32px 8px"><span style="font-size:22px;font-weight:800;color:#f4c430">Thai</span><span style="font-size:22px;font-weight:800;color:#ffffff">Mastery</span></td></tr>
    <tr><td style="padding:8px 32px 32px;color:#c9d2e8;font-size:15px;line-height:1.6">${body}</td></tr>
    <tr><td style="padding:16px 32px;background:#0d1220;font-size:12px;color:#5b6680">© ThaiMastery — Learn Thai in under 60 days.</td></tr>
  </table></body></html>`;
}
...
export const emails = {
  welcome(to: string, name: string) {
    return sendEmail({
      to,
      subject: "Welcome to ThaiMastery — your first lesson is ready 🇹🇭",
      html: layout(
  ...
  verifyEmail(to: string, token: string) {
    ...
    return sendEmail({
      to,
      subject: "Confirm your ThaiMastery email",
  ...
  passwordReset(to: string, token: string) {
    ...
    return sendEmail({
      to,
      subject: "Reset your ThaiMastery password",
  ...
  paymentReceipt(to: string, name: string, planLabel: string, amountFormatted: string) {
    return sendEmail({
      to,
      subject: "Your ThaiMastery receipt",
  ...
  abandonedCart(to: string, name: string) {
    return sendEmail({
      ...
      html: layout(`<p>Hi ${name}, you were one step away from unlocking the full ThaiMastery program. Ready to speak Thai in 60 days?</p>${button(`${siteUrl()}/en/#pricing`, "Complete my enrollment")}`),
    });
  },
};
